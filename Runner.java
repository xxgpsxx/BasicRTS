...
public class Runner extends Application implements EventHandler<InputEvent>
{
						...
						if(counter < 10)
						{
							...
							if(player.getMainBuilding().isProducing(counter))
							{
								...
								gc.fillRect(
									...
									((double)(player.getMainBuilding().getProductionTime(counter)))) * 70,
									10);

								gc.setStroke(Color.WHITE);
								Font font = Font.font("Times New Roman", FontWeight.NORMAL, 36);
								gc.setFont(font);
								gc.strokeText(
									"" + player.getMainBuilding().productionQueue(counter),
									currentBoxCoordinates[0] + 22,
	...
	public void handle(final InputEvent input)
	{
		if(input instanceof KeyEvent)
		{
			...
			{
				System.out.println(player.getArmyUnits().get(0).isAttacking());
				System.out.println(player.getSelection().size());
		}
